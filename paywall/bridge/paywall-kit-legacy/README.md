...
The sophi-react-native-paywall-kit is a Paywall SDK designed for native applications. It is implemented using React Native's legacy architecture to allow integration with both iOS and Android platforms.
...
const decider = repository.getOneByHost('www.example.com', { apiTimeout: 'value' }).then((decider) => {}).catch(() => {});
```

To get the decision call the `decider.decide` method

```js
const decision = decider.decide('content-123', 'control').then((decider) => {}).catch(() => {});
